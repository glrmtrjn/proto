...
--    txd       -- envia dados pela serial
--    rxd       -- dados vindos da serial
...
begin                                                                            
  ...
  begin
    if Reset = '0' then
     ...
     case Sreg0 is
       when S1 => if rxd = '0'  then    -- a primeira descida dos 
                       Sreg0 <= S2;     -- dados dispara o autobaud
                       ctr0 <= (OTHERS=>'0');
                  end if;
       when S2 => ctr0 <= ctr0 + 1;        -- CONTA O NÚMERO DE  PULSOS QUANDO
                  if rxd = '1' then        -- O QUE VEM DA SERIAL É '1'
                       Sreg0 <= S3;
                       Bit_cnt <= Bit_Cnt + '1';
                  end if;
       when S3 => if Bit_cnt /= "00"   and rxd = '0' then
                       Sreg0 <= S2;
                  elsif Bit_cnt = "00" and rxd = '0' then
                       Sreg0 <= S4;
                  end if;
       when S4 => if rxd = '1' then
                       Sreg0 <= S6;
                  end if;
   ...
   begin
     if ck_cycles=0 then 
        ...
        if r='0' and rxd='0' then  --- start bit
          r      <= '1';
          resync <= '1';    
          tx_av <= '0';
   ...
   --#############################################################################
   -- PARTE RELATIVA À RECEPÇÃO DOS DADOS: DESTINO PC, sinais txd e rxd_busy
   --#############################################################################
   ...
   process(rx_start, reset, serial_clk)
    ...
    txd <= word(0);   -- bit de saida, que vai para o PC
